...
public class JpegProcessor : IJpegProcessor
{
               ...
               new MemoryStream(HuffmanCodec.Decode(image.CompressedBytes, image.DecodeTable, image.BitsCount)))
        {
            for (var y = 0; y < image.Height; y += DCTSize)
            {
                for (var x = 0; x < image.Width; x += DCTSize)
                {
                    ...
                    SetPixels(result, channel, PixelFormat.YCbCr, y, x);
                }
            }
        }
    ...
    private static unsafe void SetPixels(Matrix matrix, double[,] a, PixelFormat format,
        int yOffset, int xOffset)
    {
        ...
        fixed (double* ptr = a)
        {
            ...
            for (var x = 0; x < col; x++)
                matrix.Pixels[yOffset + y, xOffset + x] = new Pixel(ptr[y * col + x], ptr[y * col + x], ptr[y * col + x], format);
        }
    }
